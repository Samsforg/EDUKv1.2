...
function transform(src) {
  ...
  function skipTemplate() { console.log("  TPL_IN", i, JSON.stringify(src.slice(i,i+30))); const start = i;
    i++;
    let sawDollar = false;
    while (i < n) {
      if (src[i] === "\\") {
      ...
      if (src[i] === "$" && src[i + 1] === "{") {
        ...
        while (i < n && depth > 0) {
          ...
          if (c === "'" || c === '"' || c === "`") console.log("  ${INNER} i", i, "c", JSON.stringify(c));
          i++;
        }
        console.log("  TPL_Interpolation_exit", start, "->", i, "depth", depth);
        continue;
      }
      i++;
    }
