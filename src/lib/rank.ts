...
export async function getGlobalRanking(limit: number, userId: number) {
  ...
  const total = (await queryOne<{ c: number }>("SELECT COUNT(*) AS c FROM users WHERE role = 'student'"))!.c;
  return {
...
export async function getCommuneRanking(commune: string, limit: number, userId: number) {
  const rows = await query<Row>(
  ...
  const total = (await queryOne<{ c: number }>("SELECT COUNT(*) AS c FROM users WHERE role = 'student' AND commune = ?", commune))!.c;
  return {
    commune,
    ranking: rankRows(rows, userId),
...
export async function getCommuneLeaders() {
  return await query<{ commune: string; count: number; total_xp: number }>(
...
export async function getLigueRanking(ligueKey: string, limit: number, userId: number) {
  const ligue = LIGUES.find((l) => l.key === ligueKey) ?? getLigueOf((await queryOne<{ xp: number }>("SELECT xp FROM users WHERE id = ?", userId))?.xp ?? 0);
  const rows = await query<Row>(
  ...
  const all = await query<{ id: number; xp: number; rank: number }>(
    `SELECT u.id, u.xp, ROW_NUMBER() OVER (ORDER BY u.xp DESC, u.last_active DESC, u.id ASC) AS rank
...
export async function getLigueLadder() {
  const entries = await Promise.all(
    LIGUES.map(async (l) => ({
      ...l,
      count: (await queryOne<{ c: number }>("SELECT COUNT(*) AS c FROM users WHERE role = 'student' AND xp >= ? AND xp < ?", l.min, l.nextMin ?? 9_999_999))!.c,
    })),
  );
  return entries;
}

export async function getAmbassadorRanking(limit: number, userId: number) {
  ...
  const myRank = await queryOne<{ rank: number; filleuls: number }>(
...
export async function getLigueStatus(userId: number) {
  const me = (await queryOne<{ id: number; xp: number; streak: number; first_name: string; last_name: string; commune: string | null }>(
    "SELECT id, xp, streak, first_name, last_name, commune FROM users WHERE id = ?",
    userId,
  ))!;
  const progress = getLigueProgress(me.xp);
  const ligueRank = await queryOne<{ rank: number }>(
    `SELECT rk.rank FROM (
  ...
  if (me.commune) {
    communeRank = await queryOne<{ rank: number; total: number }>(
