...
export async function getPromoCodes(): Promise<PromoRow[] >{
...
export async function createPromoCode(
  ...
  const existing = await queryOne<{ id: number }>("SELECT id FROM promo_codes WHERE code = ?", code);
...
export async function setPromoActive(id: number, active: boolean, actorId: number): Promise<{ ok: true } | { error: string } >{
  const row = await queryOne<{ id: number; code: string }>("SELECT id, code FROM promo_codes WHERE id = ?", id);
  if (!row) return { error: "Code promo introuvable" };
  await run("UPDATE promo_codes SET active = ? WHERE id = ?", active ? 1 : 0, id);
  await logAudit(actorId, "promo", `Code promo « ${row.code} » ${active ? "activé" : "désactivé"}`);
  return { ok: true };
}

export async function deletePromoCode(id: number, actorId: number): Promise<{ ok: true } | { error: string } >{
  const row = await queryOne<{ id: number; code: string }>("SELECT id, code FROM promo_codes WHERE id = ?", id);
  if (!row) return { error: "Code promo introuvable" };
  await run("DELETE FROM promo_codes WHERE id = ?", id);
  await logAudit(actorId, "promo", `Code promo « ${row.code} » supprimé`);
  return { ok: true };
}
