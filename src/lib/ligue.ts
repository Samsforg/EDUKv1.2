...
async function countSource(userId: number, goalType: string): Promise<number >{
  if (goalType === "quiz_done") {
    return (await queryOne<{ c: number }>("SELECT COUNT(*) AS c FROM quiz_attempts WHERE user_id = ?", userId))!.c;
  }
  ...
  if (goalType === "forum_replies") {
    return (await queryOne<{ c: number }>("SELECT COUNT(*) AS c FROM forum_replies WHERE user_id = ?", userId))!.c;
  }
  return 0;
}
...
async function grantReward(userId: number, c: Row) {
  if (c.reward_type === "xp") {
    ...
    await awardBadge(userId, c.reward_value);
  }
}

async function ensureProgress(userId: number, c: Row): Promise<void >{
  const row = await queryOne<{ progress: number; completed: number }>(
    "SELECT progress, completed FROM league_challenge_progress WHERE user_id = ? AND challenge_id = ?",
    userId,
    c.id,
  );
  if (row) return;
  const initial = Math.min(c.goal_value, await countSource(userId, c.goal_type));
...
async function toView(userId: number, c: Row, init = true): Promise<LigueChallenge >{
  if (init) await ensureProgress(userId, c);
  const row = await queryOne<{ progress: number; completed: number; completed_at: string | null }>(
...
export async function getLigueChallengesFor(userId: number) {
...
export async function creditLigueChallenges(userId: number, goalType: string, amount = 1) {
  const user = await queryOne<{ xp: number }>("SELECT xp FROM users WHERE id = ?", userId);
  if (!user) return;
  const ligue = getLigueOf(user.xp);
  const defs = await query<Row>(`SELECT ${COLUMNS} FROM league_challenges WHERE ligue = ? AND goal_type = ?`, ligue.key, goalType);
  for (const c of defs) {
    const row = await queryOne<{ progress: number; completed: number }>(
      "SELECT progress, completed FROM league_challenge_progress WHERE user_id = ? AND challenge_id = ?",
      userId,
