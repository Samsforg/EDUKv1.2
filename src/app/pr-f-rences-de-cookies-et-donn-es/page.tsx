...
export const metadata: Metadata = {
  ...
  description:
    "Gérez vos préférences de cookies et de données personnelles sur Edukora : cookies essentiels, analytiques et personnalisation IA.",
...
<a href="/" aria-label="Retour à l'accueil" className="transition-colors duration-200 active:scale-95 hover:bg-surface-container-high p-2 rounded-full">
...
<script>
        // Préférences de cookies — persistées dans localStorage + document.cookie
        const STORAGE_KEY = 'edukora_cookie_prefs';
        const toggles = &#123; analytics: document.getElementById('toggle_analytics'), ia: document.getElementById('toggle_ia') &#125;;
        const DEFAULTS = &#123; analytics: false, ia: true, essential: true &#125;;

        function applyPrefs(prefs) &#123;
            toggles.analytics.checked = !!prefs.analytics;
            toggles.ia.checked = !!prefs.ia;
            document.querySelectorAll('.toggle-checkbox').forEach(cb =&gt; &#123;
                if (!cb.disabled &amp;&amp; cb.checked) &#123;
                    const dot = cb.nextElementSibling.querySelector('.toggle-dot');
                    dot.style.transform = 'translateX(1.25rem)';
                    cb.nextElementSibling.style.backgroundColor = cb.id === 'toggle_ia' ? '#00327d' : '#0047ab';
                &#125;
            &#125;);
        &#125;
        ...
        // Charger les préférences sauvegardées, sinon les valeurs par défaut
        let prefs;
        try &#123; prefs = JSON.parse(localStorage.getItem(STORAGE_KEY)) || DEFAULTS; &#125; catch (e) &#123; prefs = DEFAULTS; &#125;
        applyPrefs(prefs);

        // Interactivité des toggles
        document.querySelectorAll('.toggle-checkbox').forEach(toggle =&gt; &#123;
            if (!toggle.disabled) &#123;
                toggle.addEventListener('change', function() &#123;
                    const dot = this.nextElementSibling.querySelector('.toggle-dot');
                    if (this.checked) &#123;
                        dot.style.transform = 'translateX(1.25rem)';
                        this.nextElementSibling.style.backgroundColor = this.id === 'toggle_ia' ? '#00327d' : '#0047ab';
                    &#125; else &#123;
                        dot.style.transform = 'translateX(0.125rem)';
                        this.nextElementSibling.style.backgroundColor = '#c3c6d5';
                    &#125;
                &#125;);
            &#125;
        &#125;);
        ...
        function collectPrefs() &#123;
            return &#123; essential: true, analytics: toggles.analytics.checked, ia: toggles.ia.checked &#125;;
        &#125;
        ...
        function showToast(btn, text) &#123;
            ...
            btn.innerText = text;
            setTimeout(() =&gt; &#123; btn.innerText = original; btn.classList.remove('opacity-80'); &#125;, 1500);
        &#125;

        document.getElementById('acceptAllBtn').addEventListener('click', function() &#123;
            prefs = &#123; essential: true, analytics: true, ia: true &#125;;
            applyPrefs(prefs);
            savePrefs(prefs);
            showToast(this, "C'est noté !");
        &#125;);

        document.getElementById('saveSelectionBtn').addEventListener('click', function() &#123;
            prefs = collectPrefs();
            savePrefs(prefs);
            showToast(this, 'Choix enregistrés !');
        &#125;);
    </script>
