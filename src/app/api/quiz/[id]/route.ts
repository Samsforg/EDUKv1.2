...
async function GETHandler(
  ...
  const quiz = await queryOne<{ id: number; title: string; subject_id: number; level: string }>(
  ...
  const questions = await query<{ id: number; question: string; options: string; points: number }>(
    "SELECT id, question, options, points FROM questions WHERE quiz_id = ? ORDER BY position",
    Number(id),
  );
  const lastAttempt = user
    ? await queryOne<{ score: number; max_score: number }>(
...
export const GET = guardApi("GET /api/quiz/[id]", GETHandler);
