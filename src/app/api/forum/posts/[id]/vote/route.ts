...
async function POSTHandler(
  ...
  if (existing) {
    await run("DELETE FROM forum_votes WHERE user_id = ? AND post_id = ?", user.id, postId);
  } else {
    await run("INSERT INTO forum_votes (user_id, post_id) VALUES (?, ?)", user.id, postId);
  }

  const votes = await queryOne<{ n: number }>(
...
export const POST = guardApi("POST /api/forum/posts/[id]/vote", POSTHandler);
