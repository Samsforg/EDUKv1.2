...
function PlansSkeleton() {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch animate-pulse">
      <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-8 flex flex-col">
        ...
        <div className="h-4 bg-surface-container-high rounded w-1/4 mb-8"></div>
        <div className="h-12 bg-surface-container-high rounded mb-8"></div>
      ...
      <div className="relative bg-primary text-on-primary rounded-xl p-8 flex flex-col shadow-xl overflow-hidden">
        <div className="absolute top-4 right-4 w-24 h-6 bg-secondary-container/50 rounded-full"></div>
        <div className="h-6 bg-primary-container/30 rounded w-1/3 mb-2"></div>
        <div className="h-4 bg-primary-container/30 rounded w-3/4 mb-6"></div>
        <div className="space-y-3 mb-6 flex-grow">
          <div className="h-5 bg-primary-container/30 rounded w-3/4"></div>
          <div className="h-5 bg-primary-container/30 rounded w-full"></div>
          <div className="h-5 bg-primary-container/30 rounded w-5/6"></div>
        </div>
        <div className="h-12 bg-primary-container/30 rounded"></div>
      </div>
    </div>
  );
}

async function fetchPlansCached(): Promise<Awaited<ReturnType<typeof import("@/lib/plans").getPremiumPlans>>> {
  const { getPremiumPlans } = await import("@/lib/plans");
  return getPremiumPlans();
}

export default async function Page() {
  const plansPromise = fetchPlansCached();
    ...
    <>
      ...
      <main role="main" className="bg-background text-on-background min-h-screen pb-24 pt-24 px-margin-mobile max-w-4xl mx-auto" style={{ minHeight: "max(884px, 100dvh)" }}>
        ...
        <Suspense fallback={<PlansSkeleton />}>
          <PlansContent plansPromise={plansPromise} />
        </Suspense>
...
async function PlansContent({ plansPromise }: { plansPromise: Promise<Awaited<ReturnType<typeof import("@/lib/plans").getPremiumPlans>>> }) {
  const { planFeatures } = await import("@/lib/plans");
