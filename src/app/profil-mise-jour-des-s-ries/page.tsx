...
<img className="w-full h-full object-cover" src="/images/ecran-288.png" alt="A professional studio portrait of a young Ivorian student" />
</div>
...
<div className="col-span-2 bg-secondary-fixed border border-secondary p-4 rounded-xl shadow-sm flex items-center justify-between">
<div className="flex items-center gap-3">
...
<img alt="Excellence Badge" className="w-full h-full object-cover rounded-full" src="/images/ecran-093.png" />
</div>
...
<input className="hidden peer" id="radio-bepc" name="exam_target" type="radio" />
<div className="flex-grow">
...
<label className="relative flex items-center p-4 bg-surface rounded-xl border-2 border-primary cursor-pointer active:scale-[0.98] transition-transform" id="label-bac">
<input checked={true} className="hidden peer" id="radio-bac" name="exam_target" type="radio" />
<div className="flex-grow">
<span className="font-body-md text-body-md text-on-surface font-semibold block">Baccalauréat (BAC)</span>
<span className="font-label-xs text-label-xs text-on-surface-variant">Enseignement Général &amp; Technique</span>
</div>
...
<div className="mt-1 animate-in fade-in slide-in-from-top-2 duration-300" id="series-selector">
<div className="bg-white border border-outline-variant rounded-xl overflow-hidden shadow-sm">
<div className="px-4 py-2 bg-surface-container-low border-b border-outline-variant">
<span className="font-label-xs text-label-xs text-primary font-bold uppercase tracking-tight">Choisir votre série</span>
</div>
<select className="w-full p-4 bg-transparent border-none focus:ring-0 text-body-md text-on-surface font-medium appearance-none">
<optgroup label="Enseignement Général">
<option value="A1">Série A1 (Littéraire)</option>
<option value="A2">Série A2 (Littéraire)</option>
<option value="C">Série C (Maths &amp; Physiques)</option>
<option selected={true} value="D">Série D (Sciences de la Vie)</option>
</optgroup>
<optgroup label="Enseignement Technique">
<option value="G2">Série G2 (Comptabilité)</option>
<option value="B">Série B (Économie)</option>
<option value="E">Série E (Maths &amp; Technique)</option>
<option value="F">Séries F (Industrielles)</option>
</optgroup>
</select>
<div className="absolute right-4 pointer-events-none mt-[-3.25rem]">
...
<script>
        const radioBac = document.getElementById('radio-bac');
        const radioBepc = document.getElementById('radio-bepc');
        ...
        function updateSelectionUI() &#123;
            labels.forEach(l =&gt; &#123;
                ...
                if (isSelected) &#123;
                    l.classList.add('border-2', 'border-primary');
                    l.classList.remove('border-outline-variant');
            ...
            // Show/Hide series selector based on BAC selection
            if (radioBac.checked) &#123;
